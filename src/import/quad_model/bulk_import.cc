...
#include "parser/import/grammar/common/value_visitor.h"
#include "parser/import/grammar/import/import_ast_adapted.h"
#include "parser/import/grammar/import/import_ast.h"
#include "parser/import/grammar/import/import_def.h"
#include "parser/import/grammar/import/import.h"
...
BulkImport::BulkImport(const string& filename) :
    catalog                         (quad_model.catalog()),
    ...
    from_edge_ordered_file          (OrderedFile<2>("from_edge_ordered_file")),
    to_edge_ordered_file            (OrderedFile<2>("to_edge_ordered_file")),
    type_edge_ordered_file          (OrderedFile<2>("type_edge_ordered_file")),
...
void BulkImport::start_import() {
    ...
    named_node_ids = std::unordered_set<uint64_t>(); // we force the destruction of the previous object to free memory
    ...
    quad_model.nodes->bulk_import(nodes_ordered_file);
...
void BulkImport::index_connections() {
    // CONNECTIONS
    from_edge_ordered_file.order(std::array<uint_fast8_t, 2> { 0, 1});
    quad_model.from_edge->bulk_import(from_edge_ordered_file);

    to_edge_ordered_file.order(std::array<uint_fast8_t, 2> { 0, 1});
    quad_model.to_edge->bulk_import(to_edge_ordered_file);

    type_edge_ordered_file.order(std::array<uint_fast8_t, 2> { 0, 1 });
    quad_model.type_edge->bulk_import(type_edge_ordered_file);

    connections_ordered_file.order(std::array<uint_fast8_t, 4> { 0, 1, 2, 3 });
    quad_model.from_to_type_edge->bulk_import(connections_ordered_file);
    ...
    quad_model.to_type_from_edge->bulk_import(connections_ordered_file);
    ...
    connections_ordered_file.order(std::array<uint_fast8_t, 4> { 2, 0, 1, 3 });
    quad_model.type_from_to_edge->bulk_import(connections_ordered_file);

    connections_ordered_file.order(std::array<uint_fast8_t, 4> { 0, 2, 1, 3 });
    quad_model.type_to_from_edge->bulk_import(connections_ordered_file);
...
void BulkImport::index_special_cases() {
    ...
    quad_model.equal_from_to->bulk_import(equal_from_to_ordered_file);

    equal_from_to_ordered_file.order(std::array<uint_fast8_t, 3> { 1, 0, 2 });
    quad_model.equal_from_to_inverted->bulk_import(equal_from_to_ordered_file);
    ...
    quad_model.equal_from_type->bulk_import(equal_from_type_ordered_file);
    ...
    quad_model.equal_from_type_inverted->bulk_import(equal_from_type_ordered_file);

    // TO=TYPE
    equal_to_type_ordered_file.order(std::array<uint_fast8_t, 3> { 0, 1, 2 });
    quad_model.equal_to_type->bulk_import(equal_to_type_ordered_file);
    ...
    quad_model.equal_to_type_inverted->bulk_import(equal_to_type_ordered_file);

    // FROM=TO=TYPE
    equal_from_to_type_ordered_file.order(std::array<uint_fast8_t, 2> { 0, 1 });
    quad_model.equal_from_to_type->bulk_import(equal_from_to_type_ordered_file);
...
void BulkImport::index_labels() {
    ...
    quad_model.node_label->bulk_import(labels_ordered_file);

    // LABEL - NODE
    labels_ordered_file.order(std::array<uint_fast8_t, 2> { 1, 0 });
    quad_model.label_node->bulk_import(labels_ordered_file);
...
void BulkImport::index_properties() {
    ...
    quad_model.object_key_value->bulk_import(properties_ordered_file);

    // KEY - VALUE - OBJECT
    properties_ordered_file.order(std::array<uint_fast8_t, 3> { 2, 0, 1 });
    quad_model.key_value_object->bulk_import(properties_ordered_file);
...
uint64_t BulkImport::process_node(const import::ast::Node& node) {
    ...
    for (auto& label : node.labels) {
        auto label_id = quad_model.get_or_create_object_id(GraphObject::make_string(label));
        ++catalog.label_count;
    ...
    for (auto& property : node.properties) {
        ...
        const auto key_id   = quad_model.get_or_create_object_id(GraphObject::make_string(property.key));
        const auto value_id = quad_model.get_or_create_object_id(value);
...
uint64_t BulkImport::create_connection(const uint64_t from_id, const uint64_t to_id, const uint64_t type_id) {
    uint64_t edge_id = ++catalog.connections_count | ObjectId::CONNECTION_MASK;
    ...
    from_edge_ordered_file.append_record(std::array<uint64_t, 2> { from_id, edge_id });
    to_edge_ordered_file.append_record(std::array<uint64_t, 2> { to_id, edge_id });
    type_edge_ordered_file.append_record(std::array<uint64_t, 2> { type_id, edge_id });
    connections_ordered_file.append_record(std::array<uint64_t, 4> { from_id, to_id, type_id, edge_id });

    quad_model.edge_table->append_record(std::array<uint64_t, 3> { from_id, to_id, type_id });
...
uint64_t BulkImport::operator()(const std::string& str) {
    if (str[0] == '_') { // Anonymous Node
        ...
        return unmasked_id | ObjectId::ANONYMOUS_NODE_MASK;
    }
    else if (str[0] == '"') { // String Literal Node
        // delete first and last characters: ("")
        std::string tmp = str.substr(1, str.size() - 2);
        auto obj_id = quad_model.get_or_create_object_id(GraphObject::make_string(tmp));
        ...
        auto obj_id = quad_model.get_or_create_object_id(GraphObject::make_named_node(str));
        if (named_node_ids.insert(obj_id).second) {
...
uint64_t BulkImport::operator()(bool b) {
    auto obj_id = quad_model.get_object_id(GraphObject::make_bool(b)).id;
    if (named_node_ids.insert(obj_id).second) {
        nodes_ordered_file.append_record({obj_id});
    }
    return obj_id;
...
uint64_t BulkImport::operator()(int64_t i) {
    auto obj_id = quad_model.get_object_id(GraphObject::make_int(i)).id;
    if (named_node_ids.insert(obj_id).second) {
        nodes_ordered_file.append_record({obj_id});
    }
    return obj_id;
...
uint64_t BulkImport::operator()(float f) {
    auto obj_id = quad_model.get_object_id(GraphObject::make_float(f)).id;
    if (named_node_ids.insert(obj_id).second) {
        nodes_ordered_file.append_record({obj_id});
    }
    return obj_id;
